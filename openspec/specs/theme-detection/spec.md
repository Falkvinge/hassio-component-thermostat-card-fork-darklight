...
The theme-detection capability covers how the card discovers whether the Home Assistant dashboard is currently rendering in dark or light mode, and how changes to that mode propagate to the card's UI. Detection reads `hass.themes.darkMode` and `hass.themes.selectedTheme` on every `hass` setter call. Selected-theme name overrides (`"Google Dark Theme"` / `"Google Light Theme"`) take precedence over the `darkMode` flag so an explicit theme choice beats auto-detection. Implementation lives in `dist/main.js` (the `resolveThemeDark` helper and its call site in the `hass` setter).
